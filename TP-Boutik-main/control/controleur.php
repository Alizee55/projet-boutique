...
class controleur {
	...
	public function produit() {
		if(isset($_GET["id"])) {
			...
			$photo=(new photo)->getPhotos($_GET["id"]);
			...
			if(count($infosArticle) > 0) {
				...
				(new vue)->produit($lesCategories, $infosArticle, $message, $photo);
	...
	public function devis(){
		...
		$lesCategories = (new categorie)->getAll();
		(new vue)->devis($lesCategories);
		$codeProduit=$_SESSION['idProduit'];
		$description= $_SESSION['description'];
		$devis=new produitDevis($description,$codeProduit);
		$devis->insert($description,$codeProduit);
		$devis->afficher();

	}
	public function repondre(){
		$lesCategories = (new categorie)->getAll();
		(new vue)->repondre($lesCategories);
		$devis=new produitDevis();
		$devis->repondre();
	}
