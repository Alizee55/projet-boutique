...
class vue {
	...
	public function produit($lesCategories, $infoArticle, $message, $lesphotos) {
		$this->entete($lesCategories);
		$_SESSION['idProduit'] = $infoArticle['codeProduit'];
		$_SESSION['description']=$infoArticle['designationProduit'];	
			...
			<div class=\"container\">
				<div class=\"row\">
					...
					foreach($lesphotos as $photo)
						{
							echo "<img src='./images/".$photo['imageProd']."' width='200'>";
						}
					echo "
					</div>";
					if ($infoArticle['type']=='Devis'){
						echo "<form action='index.php?action=devis&id=".$_SESSION['idProduit']."' method='POST' name='ajoutArticle'>
						<input type='submit' name='devis' value='Demander un devis'/>
	...
	public function categorie($lesCategories, $lesArticles, $nomCategorie) {
		...
		foreach($lesArticles as $leProduit)
		{
			...
			<img src='./images/".$leProduit['photoProduit']."' width='150'></a>";
			if($leProduit['type']=='fixe'){
				echo "<p>".$leProduit['prixProduit']."€</p></div>";
			}
			else{
				echo "<p>Tarif sur demande</p></div>";
			}
	...
	public function devis($lesCategories){
		$this->entete($lesCategories);
		echo "<form method='POST' action='index.php?action=devis'><h1>Les devis</h1></form>";

	}
	public function repondre($lesCategories){
		$this->entete($lesCategories);
	}
}